...
const ShippingInfo = () => {
  ...
  useEffect(() => {
    ...
    setShippingData(data); // ✅ Save shippingData to state
    setSubtotal(checkoutData.subtotal); // ✅ Save subtotal to state

    console.log("Shipping Data:[ShippingInfo.tsx]", shippingData);
    ...
    // Default selection: Free shipping for local pickup if available
    if (is_free_shipping_for_local_pickup) {
      ...
      const flatRateMethod = methods.find((m) => m.includes("Flat Rate"));
      if (flatRateMethod) {
        setSelectedMethod("flat_rate");
        setShippingMethod(
          "flat_rate",
          Number(flatRateMethod.split("$")[1]) || 0
        );
      }
    }
